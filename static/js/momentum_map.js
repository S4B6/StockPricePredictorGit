...
function updateMap(period) {
    ...
    fetch("/momentum/countries-performance-data/")
...
async function renderCharts() {
    try {
        ...
        const [countryData, regionData] = await Promise.all([
            fetch("/momentum/countries-performance-data").then(res => res.json()),
            fetch("/momentum/regions-performance-data").then(res => res.json())
        ]);
...
async function updateMarketStatusLights(svg, projection) {
    try {
        const response = await fetch("/momentum/market-status-data/");
        ...
        // Tooltip setup
        let tooltip = d3.select("#momentum-tooltip");
        if (tooltip.empty()) {
            tooltip = d3.select("body")
                .append("div")
                .attr("id", "momentum-tooltip")
